...
class CitaServicio:
    ...
    def reservar(self, profesional: Profesional, datos: ReservarCitaEntrada) -> Cita:
        ...
        try:
            ...
            # 2. Aviso al profesional si tiene teléfono registrado
    ...
    def reagendar(
        ...
        if cita.estado not in ESTADOS_ACTIVOS:
            raise SolicitudInvalidaExcepcion(
                f"No se puede reagendar una cita en estado '{cita.estado}'"
            )
        nuevo_fin = datos.nuevo_inicio + timedelta(minutes=profesional.duracion_cita_min)
        self._verificar_slot_libre(
            profesional.id, datos.nuevo_inicio, nuevo_fin, excluir_id=cita.id
        )
    ...
    def agregar_nota(
        self, cita: Cita, profesional_id: int, datos: NotaCitaEntrada
    ) -> NotaCita:
        nota = NotaCita(
            cita_id=cita.id,
    ...
    def slots_disponibles(
        self, profesional: Profesional, fecha: datetime
    ) -> list:
        dia_semana = fecha.weekday()
        horario = next(
            (h for h in profesional.horarios
             if h.dia_semana == dia_semana and h.esta_activo),
            None,
        )
        if not horario:
            return []
